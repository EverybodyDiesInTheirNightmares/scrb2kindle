...
抓取《四川日报》电子版**头版**全部文章正文，合并为纯文本 `txt`，并通过 SMTP 自动推送至 **Kindle**（个人文档服务原生支持 txt，无需转换、不生成 mobi）。
...
4. 组装为 UTF-8 纯文本：标题用 `【】` 包裹，每段段首两个全角空格；
...
**本地测试建议（先只测抓取、不发邮件）：** 不设置任何 SMTP 环境变量直接运行，脚本会抓取到 `scrb_front_{YYYYMMDD}.txt` 并提示「[跳过] 未检测到 SMTP 环境变量，仅生成本地 txt」。确认 `txt` 内容无误后，再配置环境变量、启用邮件推送。

## 四、GitHub Actions 每日自动推送（私有仓库）

定时任务：UTC `0 4 * * *` = **北京时间每天 12:00**（GitHub 的 `cron` 一律按 UTC 计算，中国无夏令时，故固定偏移 -8 小时）。
...
- 自动：每天 **UTC 04:00（北京时间 12:00）** 自动执行；
...
- **提示未出报**：中午 12:00 偶有报纸尚未发布，可手动重跑或微调 cron（例如改成 UTC `0 5 * * *`，即北京时间 13:00）。
